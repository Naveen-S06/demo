...
def motor_program(rpm, load, voltage, coolant_flow, rolling_radius_m, gear_ratio):
    ...
    torque.append(motor_torque_out_Nm)
    eff.append(motor_eff_percentage)
    current.append(motor_current_A)
    return veh_speed_kmph


def cycle(drive_cycle,rolling_radius_m, gear_ratio, mass, frontal_area, Kp, Ki, Kd):
    if drive_cycle == 1:
    ...
    for i,speed_kmph in enumerate(speed_array):
        ...
        for _ in range(100):
            control_signal = PIDController(Kp=0.1, Ki=2, Kd=0.0).update(adjusted_speed_kmph)
    ...
    #inp_speed_data = [item for sublist in inp_speed for item in sublist]
